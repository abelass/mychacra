<?php
	...
	function exec_formulaires() {
		...
		$config.= '</table>';
		$config.= '<table>';
		$config.= '<tr><td><h2>Résultats</h2>';
				$blocs = sql_select('id_bloc', 'spip_blocs ', "id_formulaire='$id_formulaire'");
				while ($data= sql_fetch($blocs )) {
					$id_bloc = $data['id_bloc'];
					$questions= sql_select('*', 'spip_questions ', "id_bloc='$id_bloc' AND type !='email_applicant'");
						while ($data= sql_fetch($questions )) {
						$id_question = $data['id_question'];
						$config.= '<tr><td><strong>'.  $data['titre'].'</strong></td></tr>';
						$choix= sql_select('*', 'spip_choix_question', "id_question='$id_question'");
							while ($c= sql_fetch($choix)) {
							$id_choix_question = $c['id_choix_question'];
							$titre=$c['titre'];
							$config.= '<tr><td>'.  $titre.' : </td>';
							$compteur =0;
							$resultat= sql_select('*', 'spip_reponses', "valeur='$id_choix_question'");
								while ($c= sql_fetch($resultat)) {echo '1';
								$compteur=$compteur+1;
								}
							$config.= '<td>'.  $compteur.'</td></tr>';
							}
						}
				}
		$config.= '</table>';		
